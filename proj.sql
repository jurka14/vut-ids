...
--automaticke generovani hodnot ID tabulky employee
...
INSERT INTO person          VALUES(9906274675, 'Jozef', 'Siroky', TO_DATE('28/03/1997', 'DD/MM/YYYY'), '120', 'Bratislava','Slovak Republic','421905121456');
INSERT INTO person          VALUES(9311244624, 'Jan', 'Dlouhy', TO_DATE('28/03/1997', 'DD/MM/YYYY'), '120', 'Bratislava','Slovak Republic','421905121456');
...
INSERT INTO person          VALUES(9411122182, 'Mirko', 'Tezky', TO_DATE('12/01/1987', 'DD/MM/YYYY'), '1245', 'Brno','Czech Republic','421905123759');
INSERT INTO employee        VALUES(NULL ,9905208654);
INSERT INTO employee        VALUES(NULL ,9912222320);
INSERT INTO customer        VALUES(1 ,9508150663);
INSERT INTO customer        VALUES(2 ,9906274675);
INSERT INTO customer        VALUES(3 ,9311244624);
INSERT INTO customer        VALUES(4 ,9508035933);
INSERT INTO customer        VALUES(5 ,9411122182);
...
INSERT INTO rental          VALUES(1,TO_DATE('28/04/2020', 'DD/MM/YYYY'), TO_DATE('15/5/2020', 'DD/MM/YYYY'),42, 'Karneval',1,4);
INSERT INTO rental          VALUES(2,TO_DATE('18/06/2020', 'DD/MM/YYYY'), TO_DATE('15/5/2020', 'DD/MM/YYYY'),34, 'Party',1,1);
...
INSERT INTO fine            VALUES(4,2, 30,'neskore vratenie');
INSERT INTO fine            VALUES(5,2, 30,'neskore vratenie');
INSERT INTO fine            VALUES(6,3, 30,'neskore vratenie');
...
--v tabulce employee by mela byt ID 1 a 2 misto NULL
SELECT id FROM employee;

-- ukazka druheho triggeru pri delete - ked sa vymaze rental musia sa vymazat aj vsetky fines ktoré obsahuje
SELECT * FROM fine; --tabulka pred smazanim vypujcky
DELETE FROM rental where id=1;
SELECT * FROM fine;--tabulka po smazani vypujcky


--PROCEDURE

--procedura ukaze kolik je v systemu celkove predmetu a kolik z toho je kostymu a kolik doplnku

CREATE OR REPLACE PROCEDURE count_items
AS
	vsechny_predmety NUMBER;
	kostymy NUMBER;
	doplnky NUMBER;
	num NUMBER;
	item_id item.id%TYPE;

	CURSOR my_cursor IS SELECT id FROM item;
BEGIN
	SELECT COUNT(*) INTO vsechny_predmety FROM item;
	kostymy := 0;
	doplnky := 0;
	...
	LOOP
		...
		SELECT COUNT(*) INTO num FROM costume WHERE id = item_id;
		IF num > 0 THEN
			kostymy := kostymy + 1;
		END IF;

		SELECT COUNT(*) INTO num FROM accessory WHERE id = item_id;
		IF num > 0 THEN
			doplnky := doplnky + 1;
		END IF;
	...
	DBMS_OUTPUT.put_line('Celkovy pocet predmetu: ' || vsechny_predmety );
	DBMS_OUTPUT.put_line('Pocet kostymu: ' || kostymy );
	DBMS_OUTPUT.put_line('Pocet doplnku: ' || doplnky );

END;

-- predvedeni procedury
BEGIN count_items(); END;
...
--zobraz celkovou vysi vsech pokut zakaznika se jmenem Jozef Tenky
EXPLAIN PLAN FOR

SELECT name, surname, SUM(fine.price)
FROM fine JOIN rental ON fine.id = rental.id JOIN customer ON rental.id_customer = customer.id JOIN person p on customer.birth_number = p.birth_number
WHERE name = 'Jozef' AND surname = 'Tenky'
GROUP BY p.name, p.surname;


SELECT * FROM TABLE ( DBMS_XPLAN.DISPLAY );

-- index pro id vypujcky
CREATE INDEX person_name ON person (name, surname);

--DROP INDEX person_name;
--znova s pouzitim indexu
EXPLAIN PLAN FOR
...
-- materializovany pohlad na vsetkych zakaznikov s vybranymi datami a pocet ich vypujcek
...
GRANT EXECUTE ON count_items TO xjurka08;
